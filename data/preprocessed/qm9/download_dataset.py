...
def save_qm9_tokenized_dataset(
    output_root=None,
    tokenizer_name="yairschiff/qm9-tokenizer",
    dataset_name="yairschiff/qm9",
    max_length=32,
    ...
    if "canonical_smiles" not in valid_ds.column_names:
        raise ValueError("canonical_smiles column not found in valid split")
    ...
    if force_int8:
        ...
        input_dtype = torch.int8 if max_token_id <= 127 and min_token_id >= -128 else torch.int16
...
if __name__ == "__main__":
    paths = save_qm9_tokenized_dataset(
        output_root=SCRIPT_DIR / "parsed",
        tokenizer_name="yairschiff/qm9-tokenizer",
        dataset_name="yairschiff/qm9",
        max_length=32,
