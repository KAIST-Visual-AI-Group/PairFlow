...
class TrainerBase(L.LightningModule):
  def __init__(
    ...
    self.simple_loss = self.config.algo.simple_loss
  ...
  def _validate_configuration(self):
    assert self.config.algo.backbone in {'dit', 'unet'}
  ...
  def on_validation_epoch_end(self):
    ...
    if hasattr(self.tokenizer, 'autoencoder'):
      self.tokenizer.autoencoder.to(self.device)
      self.tokenizer.autoencoder.eval()
    
    if self.config.is_vision:
      batch_size = 15
      if self.trainer.is_global_zero:
        samples = self.generate_samples(num_samples=batch_size)
        image_samples = self.tokenizer.batch_decode(samples)
  ...
  def sample_pairs(self, num_steps, eps=1e-5, x0=None):
    """Generate samples from the model."""
    # Lightning auto-casting is not working in this method for some reason
    self._eval_mode()
    xT, x0 = self.generate_samples(
      num_samples=self.config.loader.eval_batch_size,
      num_steps=num_steps,
      eps=eps,
      return_x0=True,
  ...
  def _loss(self, batch,
            ...
            simple_loss=False):
    x0 = batch['x_clean']
    valid_tokens = batch['attention_mask'] if 'attention_mask' in batch else torch.ones_like(x0)

    (input_tokens, output_tokens,
     valid_tokens) = self._process_model_input(
       x0, valid_tokens)
    loss = self.nll(batch, output_tokens,
                    current_accumulation_step, train_mode, simple_loss=simple_loss)
    assert loss.ndim == 2
    if self.ignore_bos:
      loss[:, 1:] = loss[:, 1:]
    ...
    if (self.config.training.get("compute_loss_on_pad_tokens", False) and self.training):
      token_nll = loss.mean()
    else:
      batch_nll = nlls.sum()
...
class Diffusion(TrainerBase):
  def _validate_configuration(self):
    ...
    assert self.config.sampling.noise_removal in {
      'none', 'ancestral', 'greedy', 'flow_matching'}
  ...
  def nll(self, batch, output_tokens,
          current_accumulation_step=None, train_mode=False, simple_loss=False):
    del output_tokens
    x0 = batch['x_clean']
  ...
  def generate_samples(self, num_samples, num_steps=None,
                       eps=1e-5, return_x0=False, x0=None):
    """Generate samples from the model."""
    # Lightning auto-casting is not working in this method for some reason
    ...
    if x0 is None:
      ...
      assert x0.shape == (num_samples, self.num_tokens), f"x0.shape: {x0.shape}, num_samples: {num_samples}, self.num_tokens: {self.num_tokens}"
    ...
    for i in range(num_steps):
      ...
      if self.sampler == 'ancestral':
        ...
        if (not torch.allclose(x_next, x)
          ...
          p_x0_cache = None
        x = x_next
      elif self.sampler == 'flow_matching':
        _, x = self._flow_matching_update(x=x, t=t, dt=dt, p_x0=None)
      else:
    ...
    if self.config.sampling.noise_removal == 'ancestral':
      if self.sampler == 'analytic':
        ...
        _, x = self._flow_matching_update(x=x, t=t0, dt=dt, p_x0=p_x0_cache, noise_removal_step=True)
      else:
...
class AbsorbingState(Diffusion):
  ...
  def _flow_matching_update(self, x, t, dt, p_x0=None,
    ...
    _x = torch.distributions.Categorical(probs=one_hot_x + dt * u_t).sample()
...
class UniformState(Diffusion):
  ...
  def q_xt(self, batch, alpha_t):
    ...
    # for pair datasets
    if self.config.pair == True and 'x_prior' in batch:
      xt = torch.where(move_indices, batch['x_prior'], x)
      if self.ignore_bos:
        xt[:, 0] = x[:, 0]
      return xt
  ...
  def prior_sample(self, *batch_dims, generator=None):
