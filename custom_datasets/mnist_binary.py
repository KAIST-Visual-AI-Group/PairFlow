...
class DummyVisionTokenizer:
  ...
  def batch_decode(self, x):
    return einops.rearrange(x, "b (c h w) -> b c h w", c=1,
                     h=self.image_size).repeat(1, 3, 1, 1) * 255

  def decode(self, x):
    return einops.rearrange(x, "(c h w) -> c h w", c=1,
                     h=self.image_size).repeat(3, 1, 1) * 255
